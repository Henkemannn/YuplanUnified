function createBuilderDishEditor(config) {
  ...
  function dishAllergenLabel(value) {
    ...
    if (key === "vegetarian") return "Vegetarisk";
    if (key === "vegan") return "Vegansk";
    if (key === "gluten") return "Gluten";
    if (key === "laktos") return "Laktos";
    if (key === "milk") return "Mjölk";
    if (key === "egg") return "Ägg";
    if (key === "fish") return "Fisk";
    if (key === "shellfish") return "Skaldjur";
    if (key === "nuts") return "Nötter";
    if (key === "peanuts") return "Jordnötter";
    if (key === "soy") return "Soja";
    if (key === "sesame") return "Sesam";
    if (key === "celery") return "Selleri";
    if (key === "mustard") return "Senap";
    if (key === "sulphites" || key === "sulfites") return "Sulfiter";
    if (key === "lupin") return "Lupin";
    if (key === "molluscs") return "Blötdjur";
    if (key === "crustaceans") return "Kräftdjur";
    if (key === "allium") return "Lökväxter";
    if (key === "fodmap") return "FODMAP";
    if (!key) {
      return "";
    }
  ...
  async function fetchDishLinkedComponentDetailsForCurrentComposition() {
    ...
    const componentDetails = await Promise.all(linkedComponents.map(async (linkedComponent) => {
      ...
      try {
        const detailsResponse = await _callApi("/api/builder/components/" + encodeURIComponent(componentIdValue) + "/details");
        if (!(detailsResponse && detailsResponse.status < 400 && detailsResponse.data && detailsResponse.data.ok)) {
          throw new Error("Could not load component details from backend.");
        }
        const details = detailsResponse.data && detailsResponse.data.details ? detailsResponse.data.details : {};
        return {
  ...
  async function loadDishAllergenSummaryForCurrentComposition() {
    ...
    const nextToken = Number(_getState("currentDishAllergenSummaryToken") || 0) + 1;
    _setState("currentDishAllergenSummaryToken", nextToken);
    ...
    if (nextToken !== Number(_getState("currentDishAllergenSummaryToken") || 0)) {
      return;
    }
    const currentComposition = _getState("currentBuilderComposition");
    if (!currentComposition || String(currentComposition.composition_id || "").trim() !== compositionId) {
      return;
    }
    if (String(_getState("currentBuilderDishTab") || "") !== "allergens") {
      return;
    }
