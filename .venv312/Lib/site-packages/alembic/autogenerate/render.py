...
from sqlalchemy import schema as sa_schema, sql, types as sqltypes
from sqlalchemy.sql.elements import conv, quoted_name
...
if TYPE_CHECKING:
    ...
    from alembic.util.sqla_compat import Computed, Identity
...
def _render_python_into_templatevars(
    ...
    for upgrade_ops, downgrade_ops in zip(
        migration_script.upgrade_ops_list, migration_script.downgrade_ops_list, strict=False
    ):
...
def _render_server_default(
    default: FetchedValue | str | TextClause | ColumnElement[Any] | None,
    autogen_context: AutogenContext,
    repr_: bool = True,
) -> str | None:
