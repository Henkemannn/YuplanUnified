...
from sqlalchemy import event, inspect, schema as sa_schema, text, types as sqltypes
...
if TYPE_CHECKING:
    ...
    from alembic.operations.ops import AlterColumnOp, MigrationScript, ModifyTableOps, UpgradeOps
...
def _compare_tables(
    ...
    tname_to_table = {
        ...
        for no_dflt_schema, (schema, tname) in zip(
            metadata_table_names_no_dflt_schema, metadata_table_names, strict=False
        )
    }
...
_C = TypeVar("_C", bound=UniqueConstraint | ForeignKeyConstraint | Index)
...
def _setup_autoincrement(
    ...
    if metadata_col.table._autoincrement_column is metadata_col or metadata_col.autoincrement is True:
...
def _render_server_default_for_compare(
    metadata_default: Any | None, autogen_context: AutogenContext
) -> str | None:
