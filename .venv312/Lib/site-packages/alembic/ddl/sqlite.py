...
from sqlalchemy import JSON, cast, schema, sql

from .. import util
from ..util.sqla_compat import compiles
from .base import RenameTable, alter_table, format_table_name
...
class SQLiteImpl(DefaultImpl):
    ...
    def requires_recreate_in_batch(
        ...
        for op in batch_op.batch:
            if op[0] == "add_column":
                ...
                ) and isinstance(col.server_default.arg, sql.ClauseElement) or (
    ...
    def _guess_if_default_is_unparenthesized_sql_expr(
        ...
        """
        if not expr or re.match(r"^[0-9\.]$", expr) or re.match(r"^'.+'$", expr) or re.match(r"^\(.+\)$", expr):
            return False
        else:
            return True
