...
class NamedTupleAnalyzer:
    ...
    def check_namedtuple(
        ...
        if defaults:
            default_items = {
                arg_name: default for arg_name, default in zip(items[-len(defaults) :], defaults, strict=False)
            }
        else:
            default_items = {}
    ...
    def build_namedtuple_typeinfo(
        ...
        fields = [Var(item, typ) for item, typ in zip(items, types, strict=False)]
        for var in fields:
        ...
        # are analyzed).
        vars = [Var(item, typ) for item, typ in zip(items, types, strict=False)]
