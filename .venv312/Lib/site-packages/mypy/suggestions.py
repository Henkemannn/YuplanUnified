...
class SuggestionEngine:
    ...
    def format_args(
        ...
        for i in range(len(arg_types)):
            for kind, name, typ in zip(arg_kinds[i], arg_names[i], arg_types[i], strict=False):
...
def refine_type(ti: Type, si: Type) -> Type:
    ...
    if isinstance(t, Instance) and isinstance(s, Instance) and t.type == s.type:
        return t.copy_modified(args=[refine_type(ta, sa) for ta, sa in zip(t.args, s.args, strict=False)])
    ...
    if (
        ...
        return t.copy_modified(items=[refine_type(ta, sa) for ta, sa in zip(t.items, s.items, strict=False)])
...
def refine_callable(t: CallableType, s: CallableType) -> CallableType:
    ...
    return t.copy_modified(
        arg_types=[refine_type(ta, sa) for ta, sa in zip(t.arg_types, s.arg_types, strict=False)],
