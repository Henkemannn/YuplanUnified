...
class TypeAliasType(Type):
    ...
    def _expand_once(self) -> Type:
        ...
        if self.alias.tvar_tuple_index is None:
            mapping = {v.id: s for (v, s) in zip(self.alias.alias_tvars, self.args, strict=False)}
        else:
            ...
            for tvar, sub in zip(
                self.alias.alias_tvars[:prefix] + self.alias.alias_tvars[prefix + 1 :], start + end, strict=False
            ):
                mapping[tvar.id] = sub
...
class Parameters(ProperType):
    ...
    def var_arg(self) -> FormalArgument | None:
        ...
        for position, (type, kind) in enumerate(zip(self.arg_types, self.arg_kinds, strict=False)):
            if kind == ARG_STAR:
    ...
    def argument_by_name(self, name: str | None) -> FormalArgument | None:
        ...
        for i, (arg_name, kind, typ) in enumerate(
            zip(self.arg_names, self.arg_kinds, self.arg_types, strict=False)
        ):
...
class CallableType(FunctionLike):
    ...
    def __init__(
        ...
        super().__init__(line, column)
        assert len(arg_types) == len(arg_kinds) == len(arg_names)
        for t, k in zip(arg_types, arg_kinds, strict=False):
    ...
    def copy_modified(
        ...
        *,
        ...
        if item_types is None:
            ...
            items = dict(zip(self.items, item_types, strict=False))
