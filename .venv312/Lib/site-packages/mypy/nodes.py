    ...
    def get_method(self, name: str) -> FuncBase | Decorator | None:
        for cls in self.mro:
            if name in cls.names:
                ...
                if isinstance(node, FuncBase) or isinstance(node, Decorator):
                    return node
                else:
                    return None
        return None
...
def check_arg_kinds(
    ...
    seen_named = False
    seen_opt = False
    for kind, node in zip(arg_kinds, nodes, strict=False):
        if kind == ARG_POS:
...
def check_arg_names(
    ...
    for name, node in zip(names, nodes, strict=False):
