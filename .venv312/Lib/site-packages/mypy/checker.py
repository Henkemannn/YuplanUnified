...
DeferredNodeType: _TypeAlias = FuncDef | LambdaExpr | OverloadedFuncDef | Decorator
...
class TypeChecker(NodeVisitor[None], CheckerPluginInterface):
        ...
        if fail:
            ...
            if (
                ...
                for i, (sub_kind, super_kind) in enumerate(
                    zip(override.arg_kinds, original.arg_kinds, strict=False)
                ):
        ...
        if typ.defn.type_vars and typ.defn.type_args is None:
            for base_inst in typ.bases:
                for base_tvar, base_decl_tvar in zip(
                    base_inst.args, base_inst.type.defn.type_vars, strict=False
                ):
                    if (
    ...
    def is_noop_for_reachability(self, s: Statement) -> bool:
        ...
        """
        if isinstance(s, AssertStmt) and is_false_literal(s.expr) or isinstance(s, (RaiseStmt, PassStmt)):
            return True
    ...
    def check_assignment_to_multiple_lvalues(
        ...
        if isinstance(rvalue, (TupleExpr, ListExpr)):
            ...
            if self.check_rvalue_count_in_assignment(lvalues, len(rvalues), context):
                ...
                lr_pairs = list(zip(left_lvs, left_rvs, strict=False))
                if star_lv:
                    rv_list = ListExpr(star_rvs)
                    rv_list.set_line(rvalue)
                    lr_pairs.append((star_lv.expr, rv_list))
                lr_pairs.extend(zip(right_lvs, right_rvs, strict=False))
    ...
    def check_multi_assignment_from_union(
        ...
        with self.binder.accumulate_type_assignments() as assignments:
            for item in rvalue_type.items:
                ...
                for t, lv in zip(transposed, self.flatten_lvalues(lvalues), strict=False):
        ...
        for expr, items in assignments.items():
            ...
            types, declared_types = zip(*clean_items, strict=False)
        ...
        for union, lv in zip(union_types, self.flatten_lvalues(lvalues), strict=False):
    ...
    def check_multi_assignment_from_tuple(
        ...
        if self.check_rvalue_count_in_assignment(
            ...
            for lv, rv_type in zip(left_lvs, left_rv_types, strict=False):
            ...
            for lv, rv_type in zip(right_lvs, right_rv_types, strict=False):
                self.check_assignment(lv, self.temp_node(rv_type, context), infer_lvalue_type)
        else:
    ...
    def lvalue_type_for_inference(self, lvalues: list[Lvalue], rvalue_type: TupleType) -> Type:
        ...
        def append_types_for_inference(lvs: list[Expression], rv_types: list[Type]) -> None:
            for lv, rv_type in zip(lvs, rv_types, strict=False):
    ...
    def visit_if_stmt(self, s: IfStmt) -> None:
        ...
        with self.binder.frame_context(can_skip=False, conditional_frame=True, fall_through=0):
            for e, b in zip(s.expr, s.body, strict=False):
    ...
    def visit_with_stmt(self, s: WithStmt) -> None:
        ...
        for expr, target in zip(s.expr, s.target, strict=False):
            if s.is_async:
    ...
    def visit_match_stmt(self, s: MatchStmt) -> None:
        ...
        with self.binder.frame_context(can_skip=False, fall_through=0):
            ...
            for p, g, b in zip(s.patterns, s.guards, s.bodies, strict=False):
    ...
    def _get_recursive_sub_patterns_map(
        ...
        if isinstance(expr, TupleExpr) and isinstance(typ_, TupleType):
            ...
            for item_expr, item_typ in zip(expr.items, typ_.items, strict=False):
...
def expand_callable_variants(c: CallableType) -> list[CallableType]:
    ...
    for combination in itertools.product(*tvar_values):
        tvar_map = {tv.id: subst for (tv, subst) in zip(c.variables, combination, strict=False)}
    ...
    if isinstance(t, CallableType):
        ...
        if isinstance(s, FunctionLike):
            if len(t.items) == len(s.items):
                return all(
                    is_same_arg_prefix(items, itemt) for items, itemt in zip(t.items, s.items, strict=False)
                )
    return False
