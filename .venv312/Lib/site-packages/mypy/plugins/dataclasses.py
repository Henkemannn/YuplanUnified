...
class DataclassTransformer:
    ...
    def _collect_field_args(self, expr: Expression) -> tuple[bool, dict[str, Expression]]:
        ...
        if (
            ...
            args = {}
            for name, arg, kind in zip(expr.arg_names, expr.args, expr.arg_kinds, strict=False):
...
def _meet_replace_sigs(sigs: list[CallableType]) -> CallableType:
    ...
    args = {
        name: (typ, kind)
        for name, typ, kind in zip(sigs[0].arg_names, sigs[0].arg_types, sigs[0].arg_kinds, strict=False)
    }

    for sig in sigs[1:]:
        sig_args = {
            name: (typ, kind)
            for name, typ, kind in zip(sig.arg_names, sig.arg_types, sig.arg_kinds, strict=False)
        }
