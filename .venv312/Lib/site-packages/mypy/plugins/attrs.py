            ...
            if self.converter.init_type:
                ...
                if init_type and self.init_type and self.converter.ret_type:
                    ...
                    if converter_vars and len(converter_vars) == len(init_vars):
                        variables = {
                            binder.id: arg for binder, arg in zip(converter_vars, init_vars, strict=False)
                        }
    ...
    for stmt in ctx.cls.defs.body:
        if isinstance(stmt, AssignmentStmt):
            for lvalue in stmt.lvalues:
                ...
                for lhs, rvalue in zip(lvalues, rvalues, strict=False):
...
def _add_attrs_magic_attribute(
    ...
    for (name, _), attr_type in zip(attrs, attributes_types, strict=False):
...
def _get_expanded_attr_types(
    ...
    if isinstance(typ, AnyType):
        ...
        return [dict(zip(field_names, field_types, strict=False))]
    else:
        _fail_not_attrs_class(ctx, display_typ, parent_typ)
        return None
