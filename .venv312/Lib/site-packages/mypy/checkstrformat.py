...
class StringFormatterChecker:
    ...
    def check_specs_in_format_call(
        ...
        for spec, repl in zip(specs, replacements, strict=False):
    ...
    def get_expr_by_position(self, pos: int, call: CallExpr) -> Expression | None:
        ...
        an error.
        """
        pos_args = [arg for arg, kind in zip(call.args, call.arg_kinds, strict=False) if kind == ARG_POS]
        if pos < len(pos_args):
            return pos_args[pos]
        star_args = [arg for arg, kind in zip(call.args, call.arg_kinds, strict=False) if kind == ARG_STAR]
        if not star_args:
            return None
    ...
    def get_expr_by_name(self, key: str, call: CallExpr) -> Expression | None:
        ...
        named_args = [
            arg
            for arg, kind, name in zip(call.args, call.arg_kinds, call.arg_names, strict=False)
            if kind == ARG_NAMED and name == key
        ]
        if named_args:
            return named_args[0]
        star_args_2 = [arg for arg, kind in zip(call.args, call.arg_kinds, strict=False) if kind == ARG_STAR2]
        if not star_args_2:
            return None
        ...
        if isinstance(temp_ast, MemberExpr):
            ...
            if not isinstance(temp_ast.index, (NameExpr, IntExpr)):
                ...
                self.msg.fail(
                    ...
                    f' accessor "{spec.field[len(spec.key) :]}"',
                    ctx,
                    code=codes.STRING_FORMATTING,
                )
                return False
    ...
    def check_simple_str_interpolation(
        ...
        if len(checkers) > len(rep_types):
            ...
            if len(checkers) == 1:
                ...
                for checks, rep_node in zip(checkers, replacements.items, strict=False):
                    check_node, check_type = checks
                    check_node(rep_node)
            else:
                for checks, rep_type in zip(checkers, rep_types, strict=False):
                    check_node, check_type = checks
                    check_type(rep_type)
