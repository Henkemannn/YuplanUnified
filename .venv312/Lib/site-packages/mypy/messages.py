...
class MessageBuilder:
    ...
    def maybe_note_concatenate_pos_args(
        ...
        if (
            ...
            names: list[str] = []
            for c, o in zip(
                callee_type.formal_arguments(), original_caller_type.formal_arguments(), strict=False
            ):
    ...
    def unsafe_super(self, method: str, cls: str, ctx: Context) -> None:
        self.fail(
            f'Call to abstract method "{method}" of "{cls}" with trivial body'
            " via super() is unsafe",
            ctx,
    ...
    def operator_method_signatures_overlap(
        ...
        self.fail(
            f'Signatures of "{reverse_method}" of "{reverse_class.name}" and "{forward_method}" of {format_type(forward_class, self.options)} '
    ...
    def bad_proto_variance(
        ...
        msg = capitalize(
            f'{variance_string(actual)} type variable "{tvar_name}" used in protocol where'
            f" {variance_string(expected)} one is expected"
        )
        self.fail(msg, context)
        ...
        *,
        ...
        for name, subflags, superflags in conflict_flags[:MAX_ITEMS]:
            if not class_obj and IS_CLASSVAR in subflags and IS_CLASSVAR not in superflags:
                self.note(
                    f"Protocol member {supertype.type.name}.{name} expected instance variable,"
                    " got class variable",
                    context,
                    code=code,
                )
            if not class_obj and IS_CLASSVAR in superflags and IS_CLASSVAR not in subflags:
                self.note(
                    f"Protocol member {supertype.type.name}.{name} expected class variable,"
    ...
    def generate_incompatible_tuple_error(
        ...
        error_cnt = 0
        notes: list[str] = []
        for i, (lhs_t, rhs_t) in enumerate(zip(lhs_types, rhs_types, strict=False)):
...
def format_callable_args(
    ...
    arg_strings = []
    for arg_name, arg_type, arg_kind in zip(arg_names, arg_types, arg_kinds, strict=False):
...
def format_type_inner(
    ...
    elif isinstance(typ, TypeVarType) or isinstance(typ, TypeVarTupleType):
...
def make_inferred_type_note(
    ...
    if (
        ...
        for subtype_arg, supertype_arg in zip(subtype.args, supertype.args, strict=False):
