...
def is_same_type(
    ...
    if (
        ...
        return all(is_same_type(x, y) for x, y in zip(a.args, b.args, strict=False))
...
class SubtypeVisitor(TypeVisitor[bool]):
            ...
            if (
                ...
                if right.type.has_type_var_tuple_type:
                    ...
                    if len(left_args) != len(right_args):
                        return False
                    type_params = zip(left_args, right_args, right.type.defn.type_vars, strict=False)
                else:
                    type_params = zip(t.args, right.args, right.type.defn.type_vars, strict=False)
    ...
    def visit_tuple_type(self, left: TupleType) -> bool:
        ...
        if isinstance(right, Instance):
            ...
            if len(left.items) != len(right.items):
                return False
            if any(not self._is_subtype(l, r) for l, r in zip(left.items, right.items, strict=False)):
                return False
    ...
    def variadic_tuple_subtype(self, left: TupleType, right: TupleType) -> bool:
        ...
        if left_unpack_index is None:
            ...
            if not all(
                self._is_subtype(li, ri) for li, ri in zip(prefix, right.items[:right_prefix], strict=False)
            ):
                return False
            if right_suffix and not all(
                self._is_subtype(li, ri) for li, ri in zip(suffix, right.items[-right_suffix:], strict=False)
            ):
                return False
...
def are_parameters_compatible(
    ...
    *,
    ...
    if right_star2 is not None and not trivial_suffix:
        ...
        left_only_names = set()
        for name, kind in zip(left.arg_names, left.arg_kinds, strict=False):
            if (
                name is None
                or kind.is_star()
