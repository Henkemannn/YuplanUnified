    ...
    def join_instances(self, t: Instance, s: Instance) -> ProperType:
        ...
        if t.type == s.type:
            ...
            if t.type.has_type_var_tuple_type:
                ...
                t_args = t.args
                s_args = s.args
            for ta, sa, type_var in zip(t_args, s_args, t.type.defn.type_vars, strict=False):
...
class TypeJoinVisitor(TypeVisitor[ProperType]):
    ...
    def visit_parameters(self, t: Parameters) -> ProperType:
        if isinstance(self.s, Parameters):
            ...
            return t.copy_modified(
                arg_types=[
                    meet_types(s_a, t_a) for s_a, t_a in zip(self.s.arg_types, t.arg_types, strict=False)
                ],
    ...
    def visit_instance(self, t: Instance) -> ProperType:
        ...
        elif isinstance(self.s, TypeType) or isinstance(self.s, TypedDictType) or isinstance(self.s, TupleType) or isinstance(self.s, LiteralType):
    ...
    def join_tuples(self, s: TupleType, t: TupleType) -> list[Type] | None:
        ...
        if s_unpack_index is not None and t_unpack_index is not None:
            ...
            if s.length() == t.length() and s_unpack_index == t_unpack_index:
                ...
                items = []
                for si, ti in zip(s.items[:prefix_len], t.items[:prefix_len], strict=False):
                    items.append(join_types(si, ti))
                joined = join_types(s_unpacked, t_unpacked)
                if isinstance(joined, TypeVarTupleType) or isinstance(joined, Instance) and joined.type.fullname == "builtins.tuple":
                ...
                if suffix_len:
                    for si, ti in zip(s.items[-suffix_len:], t.items[-suffix_len:], strict=False):
                        items.append(join_types(si, ti))
                return items
    ...
    def default(self, typ: Type) -> ProperType:
        ...
        elif isinstance(typ, TypedDictType) or isinstance(typ, FunctionLike):
            return self.default(typ.fallback)
        elif isinstance(typ, TypeVarType) or isinstance(typ, ParamSpecType):
...
def update_callable_ids(c: CallableType, ids: list[TypeVarId]) -> CallableType:
    tv_map = {}
    tvs = []
    for tv, new_id in zip(c.variables, ids, strict=False):
