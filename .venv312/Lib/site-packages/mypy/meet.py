...
def get_possible_variants(typ: Type) -> list[Type]:
    ...
    elif isinstance(typ, ParamSpecType) or isinstance(typ, TypeVarTupleType):
        return [typ.upper_bound]
    ...
    if isinstance(left, Instance) and isinstance(right, Instance):
        ...
        if len(left.args) == len(right.args):
            ...
            if all(
                ...
                for left_arg, right_arg in zip(left.args, right.args, strict=False)
            ):
                return True
...
def are_tuples_overlapping(
    ...
    if len(left.items) != len(right.items):
        return False
    return all(is_overlapping(l, r) for l, r in zip(left.items, right.items, strict=False))
...
class TypeMeetVisitor(TypeVisitor[ProperType]):
    ...
    def visit_parameters(self, t: Parameters) -> ProperType:
        if isinstance(self.s, Parameters):
            ...
            return t.copy_modified(
                arg_types=[join_types(s_a, t_a) for s_a, t_a in zip(self.s.arg_types, t.arg_types, strict=False)]
            )
        else:
            return self.default(self.s)
    ...
    def visit_instance(self, t: Instance) -> ProperType:
        if isinstance(self.s, Instance):
            if t.type == self.s.type:
                if is_subtype(t, self.s) or is_subtype(self.s, t):
                    ...
                    if t.type.has_type_var_tuple_type:
                        ...
                        t_args = t.args
                        s_args = self.s.args
                    for ta, sa, tv in zip(t_args, s_args, t.type.defn.type_vars, strict=False):
            ...
            if is_subtype(self.s.fallback, t):
                return self.s
            return self.default(self.s)
        elif isinstance(self.s, TypeType) or isinstance(self.s, TupleType) or isinstance(self.s, LiteralType) or isinstance(self.s, TypedDictType):
    ...
    def meet_tuples(self, s: TupleType, t: TupleType) -> list[Type] | None:
        ...
        if s_unpack_index is not None and t_unpack_index is not None:
            ...
            if s.length() == t.length() and s_unpack_index == t_unpack_index:
                ...
                if not isinstance(meet, Instance):
                    return None
                m_prefix: list[Type] = []
                for si, ti in zip(s.items[:unpack_index], t.items[:unpack_index], strict=False):
                    m_prefix.append(meet_types(si, ti))
                m_suffix: list[Type] = []
                for si, ti in zip(s.items[unpack_index + 1 :], t.items[unpack_index + 1 :], strict=False):
        ...
        items = []
        for fi, vi in zip(prefix, variadic.items[:prefix_len], strict=False):
            items.append(self.meet(fi, vi))
        for mi in middle:
            items.append(self.meet(mi, unpacked.args[0]))
        if suffix_len:
            for fi, vi in zip(suffix, variadic.items[-suffix_len:], strict=False):
                items.append(self.meet(fi, vi))
        return items
    ...
    def visit_literal_type(self, t: LiteralType) -> ProperType:
        if isinstance(self.s, LiteralType) and self.s == t or isinstance(self.s, Instance) and is_subtype(t.fallback, self.s):
            return t
        else:
            return self.default(self.s)
