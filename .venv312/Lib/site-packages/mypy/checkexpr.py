...
class ExpressionChecker(ExpressionVisitor[Type]):
    ...
    def check_typeddict_call(
        ...
        if args and all(ak in (ARG_NAMED, ARG_STAR2) for ak in arg_kinds):
            ...
            kwargs = zip([StrExpr(n) if n is not None else None for n in arg_names], args, strict=False)
    ...
    def check_callable_call(
        ...
        if var_arg and isinstance(var_arg.typ, UnpackType):
            ...
            seen_unpack = False
            for arg, arg_kind in zip(args, arg_kinds, strict=False):
    ...
    def check_argument_types(
        ...
        for i, actuals in enumerate(formal_to_actual):
            ...
            for actual, actual_type, actual_kind, callee_arg_type, callee_arg_kind in zip(
                actuals, actual_types, actual_kinds, callee_arg_types, callee_arg_kinds, strict=False
            ):
    ...
    def check_overload_call(
        ...
        if any(self.real_union(arg) for arg in arg_types):
            try:
                ...
                if unioned_return:
                    returns, inferred_types = zip(*unioned_return, strict=False)
    ...
    def plausible_overload_call_targets(
        ...
        for kind, typ in zip(arg_kinds, arg_types, strict=False):
        ...
        for typ in overload.items:
            ...
            with self.msg.filter_errors():
                if self.check_argument_count(
                    ...
                    if args_have_var_arg and typ.is_var_arg or args_have_kw_arg and typ.is_kw_arg:
    ...
    def type_overrides_set(
        ...
        for expr, typ in zip(exprs, overrides, strict=False):
    ...
    def combine_function_signatures(self, types: list[ProperType]) -> AnyType | CallableType:
        ...
        for target in callables:
            ...
            if len(new_kinds) != len(target.arg_kinds):
                too_complex = True
                break
            for i, (new_kind, target_kind) in enumerate(zip(new_kinds, target.arg_kinds, strict=False)):
    ...
    def visit_comparison_expr(self, e: ComparisonExpr) -> Type:
        ...
        for left, right, operator in zip(e.operands, e.operands[1:], e.operators, strict=False):
    ...
    def split_for_callable(
        ...
        for tv, arg in zip(t.variables, args, strict=False):
    ...
    def check_for_comp(self, e: GeneratorExpr | DictionaryComprehension) -> None:
        ...
        for index, sequence, conditions, is_async in zip(
            e.indices, e.sequences, e.condlists, e.is_async, strict=False
        ):
            if is_async:
    ...
    def visit_enum_call_expr(self, e: EnumCallExpr) -> Type:
        for name, value in zip(e.items, e.values, strict=False):
    ...
    def has_abstract_type_part(self, caller_type: ProperType, callee_type: ProperType) -> bool:
        ...
        if isinstance(caller_type, TupleType) and isinstance(callee_type, TupleType):
            return any(
                ...
                for caller, callee in zip(caller_type.items, callee_type.items, strict=False)
            )
