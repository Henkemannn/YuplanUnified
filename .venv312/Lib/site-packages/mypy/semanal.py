...
class SemanticAnalyzer(
    ...
    def check_paramspec_definition(self, defn: FuncDef) -> None:
        ...
        for arg_def, arg_kind in zip(defn.arguments, defn.arg_kinds, strict=False):
    ...
    def visit_assignment_stmt(self, s: AssignmentStmt) -> None:
        ...
        if self.check_and_set_up_type_alias(s):
            ...
            if self.process_typevar_declaration(s) or self.process_paramspec_declaration(s) or self.process_typevartuple_declaration(s) or self.analyze_namedtuple_assign(s) or self.analyze_typeddict_assign(s) or self.newtype_analyzer.process_newtype_declaration(s) or self.analyze_enum_assign(s):
                special_form = True
        ...
        if isinstance(lvalue, RefExpr):
            ...
            if isinstance(typ, TupleType):
                if len(lvalue.items) != len(typ.items):
                    ...
                    return
                for item, itemtype in zip(lvalue.items, typ.items, strict=False):
    ...
    def process_typevar_parameters(
        ...
        for param_value, param_name, param_kind in zip(args, names, kinds, strict=False):
    ...
    def process_paramspec_declaration(self, s: AssignmentStmt) -> bool:
        ...
        for param_value, param_name in zip(
            call.args[1 + n_values :], call.arg_names[1 + n_values :], strict=False
        ):
    ...
    def process_module_assignment(
        ...
        if isinstance(rval, (TupleExpr, ListExpr)) and all(
            ...
            elementwise_assignments = zip(rval.items, *[v.items for v in seq_lvals], strict=False)
    ...
    def analyze_try_stmt(self, s: TryStmt, visitor: NodeVisitor[None]) -> None:
        ...
        for type, var, handler in zip(s.types, s.vars, s.handlers, strict=False):
            if type:
                type.accept(visitor)
            if var:
    ...
    def visit_with_stmt(self, s: WithStmt) -> None:
        ...
        new_types: list[Type] = []
        for e, n in zip(s.expr, s.target, strict=False):
            e.accept(self)
            if n:
        ...
        if self.is_module_scope():
            ...
            for name in d.names:
                for table, scope_type in zip(
                    reversed(self.locals[:-1]), reversed(self.scope_stack[:-1]), strict=False
                ):
    ...
    def translate_dict_call(self, call: CallExpr) -> DictExpr | None:
        ...
        expr = DictExpr(
            [
                ...
                for key, value in zip(call.arg_names, call.args, strict=False)
            ]
        )
        expr.set_line(call)
        expr.accept(self)
    ...
    def analyze_comp_for(self, expr: GeneratorExpr | DictionaryComprehension) -> None:
        ...
        for i, (index, sequence, conditions) in enumerate(
            zip(expr.indices, expr.sequences, expr.condlists, strict=False)
        ):
            if i > 0:
    ...
    def parse_dataclass_transform_spec(self, call: CallExpr) -> DataclassTransformSpec:
        ...
        for name, value in zip(call.arg_names, call.args, strict=False):
