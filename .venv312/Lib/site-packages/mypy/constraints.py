    ...
    for i, actuals in enumerate(formal_to_actual):
        if isinstance(callee.arg_types[i], UnpackType):
            ...
            if isinstance(unpacked_type, TypeVarTupleType):
                ...
                if suffix_len:
                    for tt, at in zip(unpacked_type.items[1:], actual_types[-suffix_len:], strict=False):
    ...
    def visit_instance(self, template: Instance) -> list[Constraint]:
        ...
        if isinstance(actual, Instance):
            ...
            if self.direction == SUBTYPE_OF and template.type.has_base(instance.type.fullname):
                ...
                for tvar, mapped_arg, instance_arg in zip(tvars, mapped_args, instance_args, strict=False):
                ...
                # N.B: We use zip instead of indexing because the lengths might have
                # mismatches during daemon reprocessing.
                for tvar, mapped_arg, template_arg in zip(tvars, mapped_args, template_args, strict=False):
                    if isinstance(tvar, TypeVarType):
                        # The constraints for generic type parameters depend on variance.
                        # Include constraints from both directions if invariant.
                        if tvar.variance != CONTRAVARIANT:
...
def build_constraints_for_simple_unpack(
    ...
    for t, a in zip(template_args[:common_prefix], start, strict=False):
        res.extend(infer_constraints(t, a, direction))
    if common_suffix:
        for t, a in zip(template_args[-common_suffix:], end, strict=False):
            res.extend(infer_constraints(t, a, direction))
