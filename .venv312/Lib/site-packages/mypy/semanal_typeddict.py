...
class TypedDictAnalyzer:
    ...
    def map_items_to_base(
        ...
        for key in valid_items:
            ...
            mapped_items[key] = expand_type(
                type_in_base, {t.id: a for (t, a) in zip(tvars, base_args, strict=False)}
            )
        return mapped_items
    ...
    def analyze_typeddict_classdef_fields(
        ...
        required_keys = {
            field
            for (field, t) in zip(fields, types, strict=False)
    ...
    def check_typeddict(
        ...
        if not ok:
            ...
            required_keys = {
                field
                for (field, t) in zip(items, types, strict=False)
                if (total or (isinstance(t, RequiredType) and t.required))
                and not (isinstance(t, RequiredType) and not t.required)
            }
            types = [  # unwrap Required[T] to just T
    ...
    def build_typeddict_typeinfo(
        ...
        typeddict_type = TypedDictType(dict(zip(items, types, strict=False)), required_keys, fallback)
