...
def infer_condition_value(expr: Expression, options: Options) -> int:
    ...
    if isinstance(expr, NameExpr) or isinstance(expr, MemberExpr):
        name = expr.name
...
def infer_pattern_value(pattern: Pattern) -> int:
    if isinstance(pattern, AsPattern) and pattern.pattern is None or isinstance(pattern, OrPattern) and any(
