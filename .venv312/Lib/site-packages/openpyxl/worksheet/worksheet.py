...
class Worksheet(_WorkbookChild):
    ...
    def _move_cells(self, min_row=None, min_col=None, offset=0, row_or_col="row"):
        ...
        for row, column in sorted(self._cells, key=itemgetter(key), reverse=reverse):
            if min_row and row < min_row or min_col and column < min_col:
                continue
    ...
    def _invalid_row(self, iterable):
        raise TypeError(f"Value must be a list, tuple, range or generator, or a dict. Supplied value is {type(iterable)}"
                        )
