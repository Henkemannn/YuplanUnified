...
def dataframe_to_rows(df, index=True, header=True):
    ...
    for (df_index, row) in zip(expanded, df.itertuples(index=False), strict=False):
        row = list(row)
        if index:
            row = df_index + row
        yield row
...
def expand_index(index, header=False):
    ...
    for value in values:
        ...
        prior_change = False
        for idx, (current_index_member, previous_index_member) in enumerate(zip(value, previous_value, strict=False)):
