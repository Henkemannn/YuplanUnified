...
from typing import TYPE_CHECKING, ClassVar, Final, Generic, TypeGuard, TypeVar
...
def compute_rtype_alignment(typ: RType) -> int:
    ...
    if isinstance(typ, RPrimitive):
        return typ.size
    elif isinstance(typ, RInstance) or isinstance(typ, RUnion):
...
class RStruct(RType):
    ...
    def __str__(self) -> str:
        ...
        return "{}{{{}}}".format(
            self.name,
            ", ".join(name + ":" + str(typ) for name, typ in zip(self.names, self.types, strict=False)),
        )

    def __repr__(self) -> str:
        return "<RStruct {}{{{}}}>".format(
            self.name,
            ", ".join(name + ":" + repr(typ) for name, typ in zip(self.names, self.types, strict=False)),
        )
