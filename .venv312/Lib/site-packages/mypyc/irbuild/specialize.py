...
def any_all_helper(
    ...
    loop_params = list(zip(gen.indices, gen.sequences, gen.condlists, gen.is_async, strict=False))
...
def translate_sum_call(builder: IRBuilder, expr: CallExpr, callee: RefExpr) -> Value | None:
    ...
    loop_params = list(
        zip(gen_expr.indices, gen_expr.sequences, gen_expr.condlists, gen_expr.is_async, strict=False)
    )
    ...
    if (
        ...
        for item in expr.args[0].items:
            if isinstance(item, StrExpr):
                ...
                if not isinstance(item.callee, MemberExpr) or item.callee.name != "format" or (
