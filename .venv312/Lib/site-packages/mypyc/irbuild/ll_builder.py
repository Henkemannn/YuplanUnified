...
class LowLevelIRBuilder:
        ...
        *,
        ...
        if (src_type.is_unboxed and target_type.is_unboxed) and not is_runtime_subtype(
            ...
            if (
                ...
                return self.add(
                    TupleSet(
                        [self.coerce(v, t, line) for v, t in zip(values, target_type.types, strict=False)], line
                    )
                )
    ...
    def py_call(
        ...
        pos_args_tuple, kw_args_dict = self._construct_varargs(
            list(zip(arg_values, arg_kinds, arg_names, strict=False)), line, has_star=True, has_star2=True
        )
    ...
    def native_args_to_positional(
        ...
        *,
        ...
        star_arg_entries = []
        for lst, arg in zip(formal_to_actual, sig_args, strict=False):
            if arg.kind.is_star():
    ...
    def matching_call_c(
        ...
        for desc in candidates:
            if len(desc.arg_types) != len(args):
                continue
            if all(
                is_subtype(actual.type, formal) for actual, formal in zip(args, desc.arg_types, strict=False)
    ...
    def matching_primitive_op(
        ...
        for desc in candidates:
            ...
            if all(
                ...
                for actual, formal in zip(args, desc.arg_types, strict=False)
            ) and (not desc.is_borrowed or can_borrow):
                if matching:
                    assert matching.priority != desc.priority, f"Ambiguous:\n1) {matching}\n2) {desc}"
                    if desc.priority > matching.priority:
    ...
    def _create_dict(self, keys: list[Value], values: list[Value], line: int) -> Value:
        ...
        if size > 0:
            ...
            items = [i for t in list(zip(keys, values, strict=False)) for i in t]
