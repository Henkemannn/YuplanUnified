...
from .. import exc as sa_exc, inspect, util
...
class _AbstractLoad(traversals.GenerativeOnTraversal, LoaderOption):
    ...
    def _chop_path(
        ...
        for i, (c_token, p_token) in enumerate(
            zip(to_chop, path.natural_path, strict=False)
        ):
...
class Load(_AbstractLoad):
    ...
    def __init__(self, entity: _EntityType[Any]):
        insp = cast("Mapper[Any] | AliasedInsp[Any]", inspect(entity))
        insp._post_inspect
...
class _LoadElement(
    ...
    def _reconcile(
        ...
        if replacement._reconcile_to_other:
            return existing
        elif replacement._reconcile_to_other is False or existing._reconcile_to_other:
            return replacement
        elif existing._reconcile_to_other is False:
            return existing

        if existing is replacement or (
