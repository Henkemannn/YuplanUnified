...
def delete_obj(base_mapper, states, uowtransaction):
    ...
    for table in reversed(list(table_to_mapper.keys())):
        ...
        if table not in mapper._pks_by_table or mapper.inherits and mapper.passive_deletes:
            continue
    ...
    for (
        ...
        if (
            ...
            if bookkeeping:
                ...
                ) in zip(records, result.context.compiled_parameters, strict=False):
                    if state:
                        _postfetch(
                            mapper_rec,
                            uowtransaction,
            ...
            if do_executemany:
                ...
                if bookkeeping:
                    for (
                        ...
                        for pk, col in zip(
                            inserted_primary_key,
                            mapper._pks_by_table[table], strict=False,
                        ):
                ...
                for (
                    ...
                    for pk, col in zip(
                        primary_key, mapper._pks_by_table[table], strict=False
                    ):
                        prop = mapper_rec._columntoproperty[col]
                        if (
                            col in value_params
...
def _postfetch(
    ...
    if returning_cols:
        ...
        if row is not None:
            for row_value, col in zip(row, returning_cols, strict=False):
