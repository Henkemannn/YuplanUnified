...
from . import cache_key as _cache_key, coercions, elements, roles, schema, visitors
...
class LambdaOptions(Options):
    ...
    lambda_cache: _LambdaCacheType | None = None
...
class LambdaElement(elements.ClauseElement):
    ...
    parent_lambda: StatementLambdaElement | None = None
    closure_cache_key: tuple[Any, ...] | Literal[CacheConst.NO_CACHE]
    role: type[SQLRole]
    _rec: AnalyzedFunction | NonAnalyzedFunction
    fn: _AnyLambdaType
    ...
    def __init__(
        ...
        fn: _LambdaType,
        role: type[SQLRole],
        opts: type[LambdaOptions] | LambdaOptions = LambdaOptions,
    ...
    def _retrieve_tracker_rec(self, fn, apply_propagate_attrs, opts):
        ...
        cache_key: tuple[Any, ...] | Literal[CacheConst.NO_CACHE]
        ...
        if rec is None:
            ...
            bindparams[:] = [
                ...
                for orig_bind, new_bind in zip(
                    rec.closure_bindparams, bindparams, strict=False
                )
            ]
        ...
        if cache_key is not _cache_key.NO_CACHE:
            ...
            lambda_element: LambdaElement | None = self
    ...
    def _setup_binds_for_tracked_expr(self, expr):
        ...
        def replace(
            element: visitors.ExternallyTraversible | None, **kw: Any
    ...
    def _copy_internals(
        ...
        deferred_copy_internals: _CloneCallableType | None = None,
        **kw: Any,
    ) -> None:
...
class LinkedLambdaElement(StatementLambdaElement):
    ...
    def __init__(
        ...
        parent_lambda: StatementLambdaElement,
        opts: type[LambdaOptions] | LambdaOptions,
    ):
        self.opts = opts
        self.fn = fn
...
class AnalyzedCode:
    ...
    def _init_closure(self, fn):
        ...
        for closure_index, (fv, cell) in enumerate(
            zip(fn.__code__.co_freevars, closure, strict=False)
        ):
...
class NonAnalyzedFunction:
    ...
    closure_bindparams: list[BindParameter[Any]] | None = None
    bindparam_trackers: list[_BoundParameterGetter] | None = None
        ...
        if not build_py_wrappers:
            ...
            if closure:
                new_closure = {
                    ...
                    for fv, cell in zip(fn.__code__.co_freevars, closure, strict=False)
                }
            else:
                new_closure = {}
