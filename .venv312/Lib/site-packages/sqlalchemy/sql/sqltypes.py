...
from .elements import (
    Slice,
    TypeCoerce as type_coerce,  # noqa
    quoted_name,
)
...
_N = TypeVar("_N", bound=decimal.Decimal | float)
...
class Enum(String, SchemaType, Emulated, TypeEngine[Union[str, enum.Enum]]):
    ...
    def _setup_for_values(self, values, objects, kw):
        ...
        self._valid_lookup = dict(zip(reversed(objects), reversed(values), strict=False))

        self._object_lookup = dict(zip(values, objects, strict=False))
...
class Interval(Emulated, _AbstractInterval, TypeDecorator[dt.timedelta]):
    ...
    impl = DateTime
    epoch = dt.datetime.fromtimestamp(0, dt.UTC).replace(tzinfo=None)
    cache_ok = True
        ...
        if value is type_api._NO_VALUE_IN_LIST:
            ...
            return TupleType(
                *[
                    ...
                    for typ, elem in zip(self.types, value, strict=False)
                ]
            )
