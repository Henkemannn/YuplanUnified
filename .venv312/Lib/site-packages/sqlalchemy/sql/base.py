...
from ..util import HasMemoized as HasMemoized, hybridmethod, typing as compat_typing
from ..util.typing import Protocol, Self, TypeGuard
from . import roles, visitors
from .cache_key import (
    HasCacheKey,  # noqa
    MemoizedHasCacheKey,  # noqa
)
...
def _from_objects(
    *elements: ColumnElement[Any] | FromClause | TextClause | _JoinTargetElement
) -> Iterator[FromClause]:
...
_COLKEY = TypeVar("_COLKEY", None | str, str)
...
class _ColumnMetrics(Generic[_COL_co]):
    ...
    def embedded(
        self,
        target_set: set[ColumnElement[Any]] | frozenset[ColumnElement[Any]],
    ) -> bool:
...
class ColumnCollection(Generic[_COLKEY, _COL_co]):
    ...
    @overload
    def get(self, key: str, default: None = None) -> _COL_co | None: ...

    @overload
    def get(self, key: str, default: _COL) -> _COL_co | _COL: ...

    def get(
        self, key: str, default: _COL | None = None
    ) -> _COL_co | _COL | None:
...
class DedupeColumnCollection(ColumnCollection[str, _NAMEDCOL]):
    ...
    def add(  # type: ignore[override]
        self, column: _NAMEDCOL, key: str | None = None
    ) -> None:
    ...
    def _populate_separate_keys(
        ...
        for k, col in cols:
            ...
            if col.name in self._index and col.key != col.name or col.key in self._index:
