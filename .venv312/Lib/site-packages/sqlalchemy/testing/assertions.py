...
from .. import exc as sa_exc, schema, sql, types as sqltypes, util
...
class AssertsCompiledSQL:
    def assert_compile(
        ...
        if check_param_order and getattr(c, "params", None):
            ...
            expected = [v for _, v in sorted(zip(positions, values, strict=False))]
...
class ComparesTables:
    def assert_tables_equal(
        ...
        for c, reflected_c in zip(table.c, reflected_table.c, strict=False):
...
class AssertsExecutionResults:
    ...
    def assert_statement_count_multi_db(self, dbs, counts):
        recs = [
            ...
            for (db, count) in zip(dbs, counts, strict=False)
        ]
        asserters = []
        ...
        try:
            ...
            for asserter, (ctx, db, count) in zip(asserters, recs, strict=False):
        ...
        for exp in expected:
            ...
            if exprs:
                ...
                for idx_exp, expr, col in zip(
                    idx.expressions, exprs, exp["column_names"], strict=False
                ):
                    if col is None:
