        ...
        for table, fks in sort_tables_and_constraints(
            ...
            with cls.bind.begin() as conn:
                conn.execute(
                    table.insert(),
                    [
                        dict(zip(headers[table], column_values, strict=False))
...
class CacheKeyFixture:
    def _compare_equal(self, a, b, compare_values):
        ...
        if a_key is None:
            ...
            for a_param, b_param in zip(a_key.bindparams, b_key.bindparams, strict=False):
            ...
            if a == b:
                ...
                if a_key.key == b_key.key:
                    for a_param, b_param in zip(
                        a_key.bindparams, b_key.bindparams, strict=False
                    ):
