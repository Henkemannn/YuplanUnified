...
class immutabledict(ImmutableDictBase[_KT, _VT]):
    ...
    def __init__(
        self, *args: Mapping[_KT, _VT] | Iterable[tuple[_KT, _VT]]
    ):
        pass
    ...
    def union(
        self, __d: Mapping[_KT, _VT] | None = None
    ...
    def _union_w_kw(
        self, __d: Mapping[_KT, _VT] | None = None, **kw: _VT
    ) -> immutabledict[_KT, _VT]:
...
class OrderedSet(set[_T]):
    ...
    def union(self, *other: Iterable[_S]) -> OrderedSet[_T | _S]:
        result: OrderedSet[_T | _S] = self.copy()
        result.update(*other)
        return result
    ...
    def __xor__(self, other: AbstractSet[_S]) -> OrderedSet[_T | _S]:
        return cast(OrderedSet[_T | _S], self).symmetric_difference(
            other
        )
    ...
    def __isub__(self, other: AbstractSet[_T | None]) -> OrderedSet[_T]:  # type: ignore  # noqa: E501
...
def unique_list(
    seq: Iterable[_T], hashfunc: Callable[[_T], int] | None = None
) -> list[_T]:
    seen: set[Any] = set()
    seen_add = seen.add
    if not hashfunc:
