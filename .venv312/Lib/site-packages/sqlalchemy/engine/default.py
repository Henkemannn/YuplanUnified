...
from ..sql import compiler, dml, expression, type_api, util as sql_util
...
from . import characteristics, cursor as _cursor, interfaces
        ...
        for imv_batch in compiled._deliver_insertmanyvalues_batches(
            ...
            if is_returning:
                ...
                if imv.num_sentinel_columns and not imv_batch.is_downgraded:
                        ...
                        if composite_sentinel:
                            _composite_sentinel_proc = [
                                ...
                                for col, cursor_desc in zip(
                                    ...
                                    cursor.description[-_nsc:], strict=False,
                                )
                            ]
                        else:
                    ...
                    rows_by_sentinel: dict[tuple[Any, ...], Any] | dict[Any, Any]
                    if composite_sentinel:
                        rows_by_sentinel = {
                            tuple(
                                (proc(val) if proc else val)
                                for val, proc in zip(
                                    row[-_nsc:], _composite_sentinel_proc, strict=False
                                )
                            ): row
                            for row in rows
                        }
...
class DefaultExecutionContext(ExecutionContext):
    ...
    result_column_struct: tuple[list[ResultColumnsEntry], bool, bool, bool, bool] | None = None
    ...
    def _init_statement(
        ...
        if not parameters:
            if self.dialect.positional:
                ...
                self.parameters = [self._empty_dict_params]
        elif isinstance(parameters[0], dialect.execute_sequence_format) or isinstance(parameters[0], dict):
    ...
    def _setup_ins_pk_from_implicit_returning(self, result, rows):
        ...
        return [
            getter(row, param) for row, param in zip(rows, compiled_params, strict=False)
        ]
