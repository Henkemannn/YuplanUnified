...
import typing
import weakref
from collections.abc import Collection, Iterator, MutableMapping, MutableSequence, Sequence
...
_ListenerFnSequenceType = Union[collections.deque[_T], _empty_collection[_T]]
...
class _ClsLevelDispatch(RefCollection[_ET]):
    ...
    def _wrap_fn_for_kw(self, fn: _ListenerFnType) -> _ListenerFnType:
        def wrap_kw(*args: Any, **kw: Any) -> Any:
            argdict = dict(zip(self.arg_names, args, strict=False))
            argdict.update(kw)
            return fn(**argdict)
...
class _ListenerCollection(_CompoundListener[_ET]):
    ...
    parent: _ClsLevelDispatch[_ET]
    name: str
    listeners: collections.deque[_ListenerFnType]
