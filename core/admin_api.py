...
def admin_users_create_stub():  # type: ignore[return-value]
    ...
    try:
        ...
        try:
            # Set updated_at to current UTC to seed a stable ETag
            from datetime import UTC as _UTC, datetime as _dt
            try:
                new_user.updated_at = _dt.now(_UTC)
            except Exception:
                pass
            db.add(new_user)
            db.commit()
        ...
        try:
            ...
            pass
        resp_body = {"id": str(getattr(new_user, "id", "")), "email": str(email), "role": str(role)}
        resp = jsonify(resp_body)
        try:
            resp.headers["ETag"] = user_etag(getattr(new_user, "id", None), getattr(new_user, "updated_at", None))
        except Exception:
            pass
        return resp, 201
    except Exception:
...
def admin_users_delete_soft(user_id: str):  # type: ignore[return-value]
    ...
    Behavior:
      ...
      - If found → set deleted_at=now(UTC), commit, emit audit, return 204 No Content.
    ...
    try:
        ...
        if not row:
            r404 = jsonify({"ok": False, "error": "not_found", "message": "user not found"})
            return r404, 404  # type: ignore[return-value]
        # Concurrency: require If-Match header to match current ETag (always for DELETE)
        got = request.headers.get("If-Match")
        try:
            expected = user_etag(getattr(row, "id", None), getattr(row, "updated_at", None))
        except Exception:
            expected = 'W/"0"'
        tags = parse_if_match(got)
        if "*" in tags:
            pass
        else:
            norm_expected = expected
            # weak compare: strip W/"" and quotes for both sides
            norm_expected = norm_expected.replace('W/"', '"')
            bare_expected = norm_expected.strip('"')
            match = bare_expected in {t.strip('"') for t in tags}
            if not match:
                return precondition_failed(expected, got, resource_id=getattr(row, "id", None))
        # Soft-delete
        ...
        try:
            ...
            pass
        # 204 No Content per Phase 3 contract
        return "", 204
    finally:
        db.close()
...
def admin_users_update_patch(user_id: str):  # type: ignore[return-value]
    ...
    try:
        ...
        if not row:
            r404 = jsonify({"ok": False, "error": "not_found", "message": "user not found"})
            return r404, 404  # type: ignore[return-value]
        # NOTE: For PATCH, enforce If-Match only when a change would be applied (idempotent no-change is allowed)
        ...
        from datetime import UTC as _UTC, datetime as _dt
        if changed:
            # Concurrency: require If-Match header to match current ETag
            got = request.headers.get("If-Match")
            try:
                tags = parse_etag_header(got, allow_star=True)
            except ValueError:
                return invalid_header_problem("If-Match")
            try:
                expected = user_etag(getattr(row, "id", None), getattr(row, "updated_at", None))
            except Exception:
                expected = 'W/"0"'
            if "*" not in tags:
                norm_expected = expected.replace('W/"', '"')
                bare_expected = norm_expected.strip('"')
                match = bare_expected in {t.strip('"') for t in tags}
                if not match:
                    return precondition_failed(expected, got, resource_id=getattr(row, "id", None))
        ...
        _ua = getattr(row, "updated_at", None)
        _ua_str = _ua.isoformat() if isinstance(_ua, _dt) else None
        resp_body = {
        ...
        resp = jsonify(resp_body)
        try:
            resp.headers["ETag"] = user_etag(getattr(row, "id", None), getattr(row, "updated_at", None))
        except Exception:
            pass
        return resp, 200
    finally:
...
def admin_users_get(user_id: str):
    ...
    Behavior:
      - Returns 200 with {id,email,role,updated_at} and ETag header.
      - If If-None-Match matches current ETag (including wildcard *), returns 304 with no body.
      - 404 if not found (tenant scoped, active only).
    """
    from flask import g as _g
    ...
    try:
        ...
        try:
            current_etag = user_etag(getattr(row, "id", None), getattr(row, "updated_at", None))
        except Exception:
            current_etag = 'W/"0"'
        # If-None-Match handling for caching
        inm = request.headers.get("If-None-Match")
        try:
            tags = parse_etag_header(inm, allow_star=True)
        except ValueError:
            return invalid_header_problem("If-None-Match")
        if "*" in tags:
            return "", 304
        norm_expected = current_etag.replace('W/"', '"').strip('"')
        if norm_expected in {t.strip('"') for t in tags}:
            return "", 304
        # Otherwise return representation with ETag
        _ua = getattr(row, "updated_at", None)
        _ua_str = _ua.isoformat() if isinstance(_ua, _dt) else None
        body = {
            "id": str(getattr(row, "id", user_id)),
...
def admin_users_replace_put(user_id: str):  # type: ignore[return-value]
    ...
    try:
        ...
        if not row:
            r404 = jsonify({"ok": False, "error": "not_found", "message": "user not found"})
            return r404, 404  # type: ignore[return-value]
        # NOTE: For PUT, enforce If-Match only when a change would be applied (idempotent no-change is allowed)
...
def admin_feature_flag_update_stub(key: str):  # type: ignore[return-value]
    ...
    if isinstance(data, dict):
        ...
        if "notes" in data:
            val = data.get("notes")
            # Accept None (treated as no-op) and strings up to 500 chars
    ...
    # Persist enabled/notes if provided; return updated record with ETag and If-Match enforcement on change
    try:
        from datetime import UTC as _UTC, datetime as _dt
        ...
        if row2 is not None:
            ...
            # Only mark changed when the incoming value differs from current state
            if isinstance(data, dict) and "enabled" in data:
                new_enabled = bool(data.get("enabled"))
                if bool(getattr(row2, "enabled", False)) != new_enabled:
                    row2.enabled = new_enabled
                    changed = True
                    change_fields["enabled"] = new_enabled
            if isinstance(data, dict) and "notes" in data:
                val = data.get("notes")
                new_notes = str(val) if isinstance(val, str) else None
                curr_notes_norm = (getattr(row2, "notes", None) or "")
                new_notes_norm = (new_notes or "")
                if curr_notes_norm != new_notes_norm:
                    row2.notes = new_notes
                    changed = True
                    change_fields["notes"] = new_notes
            if changed:
                # Concurrency: require If-Match to match current ETag for this flag
                got = request.headers.get("If-Match")
                try:
                    tags = parse_etag_header(got, allow_star=True)
                except ValueError:
                    ...
                    return invalid_header_problem("If-Match")
                try:
                    expected = user_etag(getattr(row2, "name", str(key)), getattr(row2, "updated_at", None))
                except Exception:
                    expected = 'W/"0"'
                if "*" not in tags:
                    if expected.replace('W/"', '"').strip('"') not in {t.strip('"') for t in tags}:
                        db2.close()
                        return precondition_failed(expected, got, resource="feature_flag", resource_id=str(getattr(row2, "name", str(key))))
                ...
                try:
                    ...
                    pass
            _ua = getattr(row2, "updated_at", None)
            _ua_str = _ua.isoformat() if isinstance(_ua, _dt) else None
            resp_body = {
                "key": str(row2.name),
            ...
            resp = jsonify(resp_body)
            try:
                resp.headers["ETag"] = user_etag(getattr(row2, "name", str(key)), getattr(row2, "updated_at", None))
            except Exception:
                pass
            return resp, 200
...
@bp.get("/feature-flags/<string:key>")
@require_roles("admin")
def admin_feature_flag_get(key: str):  # type: ignore[return-value]
    from flask import g as _g
    ...
    try:
        try:
            ...
            tid_int = None
        if tid_int is None:
            return jsonify({"ok": False, "error": "not_found", "message": "feature flag not found"}), 404  # type: ignore[return-value]
        row = db.query(_TFF).filter_by(tenant_id=tid_int, name=str(key)).first()
        if not row:
            return jsonify({"ok": False, "error": "not_found", "message": "feature flag not found"}), 404  # type: ignore[return-value]
        try:
            etag = user_etag(getattr(row, "name", str(key)), getattr(row, "updated_at", None))
        except Exception:
            etag = 'W/"0"'
        inm = request.headers.get("If-None-Match")
        try:
        ...
        if "*" in tags:
            return "", 304
        if etag.replace('W/"', '"').strip('"') in {t.strip('"') for t in tags}:
            return "", 304
        _ua = getattr(row, "updated_at", None)
        _ua_str = _ua.isoformat() if isinstance(_ua, _dt) else None
        body = {
            "key": str(getattr(row, "name", key)),
...
@bp.delete("/feature-flags/<string:key>")
@require_roles("admin")
def admin_feature_flag_delete(key: str):  # type: ignore[return-value]
    from flask import g as _g
    ...
    try:
        ...
        try:
            ...
            return invalid_header_problem("If-Match")
        try:
            expected = user_etag(getattr(row, "name", str(key)), getattr(row, "updated_at", None))
        except Exception:
            expected = 'W/"0"'
        if "*" not in tags:
            if expected.replace('W/"', '"').strip('"') not in {t.strip('"') for t in tags}:
                return precondition_failed(expected, got, resource="feature_flag", resource_id=str(getattr(row, "name", str(key))))
        # Delete the override row
        db.delete(row)
        db.commit()
        try:
            _emit_audit("feature_flag_delete", tenant_id=tid_int, key=str(key))
        except Exception:
            pass
        return "", 204
    finally:
        ...
        if row2 is not None:
            ...
            if changed:
                ...
                try:
                    ...
                    return invalid_header_problem("If-Match")
                try:
                    expected = user_etag(getattr(row2, "id", None), getattr(row2, "updated_at", None))
                except Exception:
                    expected = 'W/"0"'
                if "*" not in tags:
                    norm_expected = expected.replace('W/"', '"')
                    ...
                    if not match:
                        db2.close()
                        return precondition_failed(expected, got, resource_id=getattr(row2, "id", None))
                # Apply change and bump timestamp
                row2.role = new_role
                try:
                    row2.updated_at = _dt.now(_UTC)
                except Exception:
            ...
            resp = jsonify(resp_body)
            try:
                resp.headers["ETag"] = user_etag(getattr(row2, "id", None), getattr(row2, "updated_at", None))
            except Exception:
                pass
            db2.close()
            return resp, 200
...
@bp.get("/roles/<string:user_id>")
@require_roles("admin")
def admin_roles_get(user_id: str):  # type: ignore[return-value]
    from flask import g as _g
    ...
    try:
        ...
        if tid_int is None or uid_int is None:
            r404 = jsonify({"ok": False, "error": "not_found", "message": "user not found"})
            return r404, 404  # type: ignore[return-value]
        row = db.query(_User).filter(_User.tenant_id == tid_int, _User.id == uid_int, _User.deleted_at.is_(None)).first()
        if not row:
            r404 = jsonify({"ok": False, "error": "not_found", "message": "user not found"})
            return r404, 404  # type: ignore[return-value]
        try:
            etag = user_etag(getattr(row, "id", None), getattr(row, "updated_at", None))
        except Exception:
            etag = 'W/"0"'
        inm = request.headers.get("If-None-Match")
        try:
...
@bp.delete("/roles/<string:user_id>")
@require_roles("admin")
def admin_roles_delete(user_id: str):  # type: ignore[return-value]
    from datetime import UTC as _UTC, datetime as _dt
    ...
    try:
        ...
        uid_int = None
    if tid_int is None or uid_int is None:
        return jsonify({"ok": False, "error": "not_found", "message": "user not found"}), 404  # type: ignore[return-value]
    db = _get_session()
    try:
        row = db.query(_User).filter(_User.tenant_id == tid_int, _User.id == uid_int, _User.deleted_at.is_(None)).first()
        if not row:
        ...
        db.commit()
        try:
            _emit_audit("user_delete", tenant_id=tid_int, user_id=getattr(row, "id", None))
        except Exception:
            pass
        return "", 204
    finally:
