...
class AttendanceService:
    ...
    def set_attendance(
        self, unit_id: int, day_date: date, meal: str, count: int, origin: str = "manual"
    ) -> int:
        meal = meal.strip()
        db = get_session()
        try:
            row: Attendance | None = (
                db.query(Attendance).filter_by(unit_id=unit_id, date=day_date, meal=meal).first()
            )
            if row is not None:
                row.count = count
                row.origin = origin
                ...
                row = Attendance(
                    unit_id=unit_id, date=day_date, meal=meal, count=count, origin=origin
                )
                db.add(row)
            db.commit()
    ...
    def summary(self, unit_id: int, start: date, end: date) -> dict[str, Any]:
        ...
        try:
            q = db.query(Attendance).filter(
                Attendance.unit_id == unit_id, Attendance.date >= start, Attendance.date <= end
            )
            items = []
