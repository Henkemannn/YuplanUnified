...
class ExcelMenuImporter(MenuImporter):
    ...
    def parse(self, file_bytes: bytes, filename: str) -> MenuImportResult:
        if pd is None:
            return MenuImportResult(weeks=[], errors=["pandas not installed"], warnings=["Excel import disabled"])
        ...
        if not weeks:
            return MenuImportResult(weeks=[], errors=["No sheets matching 'Uke <n>' found"], warnings=[])
    ...
    def _extract(self, df, week: int):
        ...
        for i in range(len(df)):
            ...
            if self._valid_row(day, cat, dish):
                items.append(ImportedMenuItem(day=_DAY_MAP[day.lower()], meal="lunch", variant_type="main", dish_name=str(dish).strip(), category=str(cat).strip() or None, source_labels=["lunch"]))
