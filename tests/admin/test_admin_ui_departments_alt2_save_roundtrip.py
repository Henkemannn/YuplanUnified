...
def test_admin_alt2_save_roundtrip_legacy_schema(app_session, client_admin: FlaskClient):
    from core.db import get_session
    ...
    try:
        ...
        conn.execute(text("CREATE TABLE weekview_alt2_flags(tenant_id TEXT, department_id TEXT, year INTEGER, week INTEGER, day_of_week INTEGER, is_alt2 INTEGER, UNIQUE(tenant_id, department_id, year, week, day_of_week))"))
        conn.execute(text("INSERT OR IGNORE INTO sites(id, name) VALUES ('site-alt2-legacy-save','Alt2 Legacy Save Site')"))
    ...
    r_save = client_admin.post(
        ...
        headers=ADMIN_HEADERS,
        json={"year": 2026, "week": 12, "alt2_days": ["mon", "wed"]},
    )
    assert r_save.status_code in (200, 201)
    ...
    r_get = client_admin.get(
        "/ui/admin/departments/dept-alt2-legacy-save/alt2?year=2026&week=12",
        headers=ADMIN_HEADERS,
    )
    assert r_get.status_code == 200
