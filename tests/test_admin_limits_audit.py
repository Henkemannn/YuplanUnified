...
from core.admin_api import bp as admin_bp
...
def test_upsert_audited(monkeypatch):
    ...
    c = app.test_client()
    resp = c.post('/admin/limits', headers={'X-User-Role':'admin'}, json={"tenant_id":5,"name":"exp","quota":9,"per_seconds":60})
...
def test_delete_audited(monkeypatch):
    ...
    c = app.test_client()
    # create then delete
    resp1 = c.post('/admin/limits', headers={'X-User-Role':'admin'}, json={"tenant_id":6,"name":"exp","quota":3,"per_seconds":60})
    resp2 = c.delete('/admin/limits', headers={'X-User-Role':'admin'}, json={"tenant_id":6,"name":"exp"})
