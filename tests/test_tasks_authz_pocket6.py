...
def test_tasks_editor_create_and_list(client_admin):
    create = client_admin.post("/tasks/", json={"title": "Hello"}, headers={"X-User-Role": "editor", "X-Tenant-Id": "1"})
...
def test_tasks_update_tenant_mismatch(client_admin):
    ...
    create = client_admin.post("/tasks/", json={"title": "Mismatch"}, headers={"X-User-Role": "editor", "X-Tenant-Id": "1"})
    assert create.status_code == 201
    tid_task_id = create.get_json()["task"]["id"] if create.get_json().get("task") else create.get_json().get("task_id")
