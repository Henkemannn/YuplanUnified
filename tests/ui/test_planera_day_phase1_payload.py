...
def _seed(db, site_id: str, dep_id: str, date_str: str):
    ...
    db.execute(text("INSERT INTO menus(tenant_id, year, week, status, updated_at) VALUES(1,2025,47,'draft','2025-11-20T12:00:00Z')"))
    menu_id_row = db.execute(text("SELECT id FROM menus WHERE tenant_id=1 AND year=2025 AND week=47 ORDER BY id DESC LIMIT 1")).fetchone()
    menu_id = menu_id_row[0] if menu_id_row else 1
    ...
    for day in ["Måndag"]:
        db.execute(text("INSERT OR REPLACE INTO menu_variants(menu_id, day, meal, variant_type, dish_id) VALUES(:m,:d,'Lunch','alt1',NULL)"), {"d": day, "m": menu_id})
        db.execute(text("INSERT OR REPLACE INTO menu_variants(menu_id, day, meal, variant_type, dish_id) VALUES(:m,:d,'Lunch','alt2',NULL)"), {"d": day, "m": menu_id})
        db.execute(text("INSERT OR REPLACE INTO menu_variants(menu_id, day, meal, variant_type, dish_id) VALUES(:m,:d,'Kväll','alt1',NULL)"), {"d": day, "m": menu_id})
    # Residents
