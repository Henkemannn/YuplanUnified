"""
Leak test: ensure department list is strictly site-scoped.
- Seed two sites A and B
- Create a department only in site A
- With active site B, list should not show site A department
- With active site A, list should show it
"""
import uuid
...
def test_admin_departments_list_strict_site_scope(app_session, client_admin):
    app = app_session
    ...
    # Activate site B and verify the list does NOT show Dept A
    with client_admin.session_transaction() as sess:
        sess["site_id"] = site_b
    r_b = client_admin.get("/ui/admin/departments", headers=_h("admin"))
    assert r_b.status_code == 200
    html_b = r_b.data.decode("utf-8")
    assert "Dept A" not in html_b

    # Switch to site A and verify the list shows Dept A
    with client_admin.session_transaction() as sess:
        sess["site_id"] = site_a
    r_a = client_admin.get("/ui/admin/departments", headers=_h("admin"))
