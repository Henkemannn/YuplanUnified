...
def _seed_portal_week(db, dept_id: str, site_id: str, year: int, week: int):
    ...
    db.execute(text("CREATE TABLE IF NOT EXISTS menus(id INTEGER PRIMARY KEY, tenant_id INTEGER NOT NULL, week INTEGER, year INTEGER, status TEXT)"))
    ...
    db.execute(text("DELETE FROM dishes WHERE id IN (801,802,803,804)"))
    db.execute(text("INSERT OR REPLACE INTO dishes(id,tenant_id,name,category) VALUES(801,1,'Pannbiff',NULL)"))
    db.execute(text("INSERT OR REPLACE INTO dishes(id,tenant_id,name,category) VALUES(802,1,'Köttbullar',NULL)"))
    db.execute(text("INSERT OR REPLACE INTO dishes(id,tenant_id,name,category) VALUES(803,1,'Fruktsallad',NULL)"))
    db.execute(text("INSERT OR REPLACE INTO dishes(id,tenant_id,name,category) VALUES(804,1,'Kvällsgröt',NULL)"))
    ...
    db.execute(text("INSERT INTO menu_variants(menu_id,day,meal,variant_type,dish_id) VALUES(901,'mon','dessert','dessert',803)"))
    db.execute(text("INSERT INTO menu_variants(menu_id,day,meal,variant_type,dish_id) VALUES(901,'mon','dinner','dinner',804)"))
    db.commit()


def test_global_navigation_present(client_admin):
    year = 2025
    week = 47
