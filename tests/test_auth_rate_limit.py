...
def test_auth_rate_limit_basic(monkeypatch):
    cfg = {"TESTING": True, "SECRET_KEY": "x", "AUTH_RATE_LIMIT": {"window_sec": 60, "max_failures": 3, "lock_sec": 2}}
    app = create_app(cfg)
    ...
    u = User(tenant_id=t.id, email=user_email, password_hash=generate_password_hash("pw"), role="admin", unit_id=None)
    db.add(u)
    db.commit()
    db.close()
