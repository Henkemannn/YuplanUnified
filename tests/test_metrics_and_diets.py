...
def make_app(tmp_path, name="test"):
    ...
    app = create_app({"database_url": db_url, "secret_key": "test", "TESTING": True})
...
def test_diet_type_crud_and_feature_toggle(tmp_path):
    ...
    with app.test_client() as client:
        ...
        rv = client.post(
            ...
            json={"feature": "menus", "enabled": False, "tenant_id": new_tid},
        )
        assert rv.status_code == 200 and rv.get_json()["enabled"] is False
        rv = client.post(
            "/admin/features/toggle",
            json={"feature": "menus", "enabled": True, "tenant_id": new_tid},
        )
        ...
        # Update
        rv = client.post(
            f"/diet/types/{diet_id}", json={"name": "Glutenfri Uppd", "default_select": False}
        )
        assert rv.status_code == 200
        rv = client.get("/diet/types")
        types = rv.get_json()["diet_types"]
        assert any(
            t["id"] == diet_id and t["name"] == "Glutenfri Uppd" and (not t["default_select"])
            for t in types
        )
        # Delete
...
def test_service_metrics_ingest_and_summary(tmp_path):
    ...
    with app.test_client() as client:
        ...
        # Query
        rv = client.post(
            "/metrics/query", json={"filters": {"date_from": "2025-09-28", "date_to": "2025-09-28"}}
        )
        ...
        row = summ[0]
    assert (
        row["produced_qty_kg"] == 7.0
        and row["served_qty_kg"] == 5.7
        and row["leftover_qty_kg"] == 1.5
    )
