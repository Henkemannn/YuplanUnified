...
def test_feature_flag_flow(client_admin):
    ...
    resp = client_admin.post(
        "/features/set",
        json={"name": flag_name, "enabled": True},
    ...
    resp = client_admin.get(
        f"/features/check?name={flag_name}", headers={"X-User-Role": "admin", "X-Tenant-Id": "1"}
    )
    assert resp.status_code == 200
    assert resp.json["enabled"] is True
    ...
    resp = client_admin.post(
        "/features/set",
        json={"name": flag_name, "enabled": False},
        headers={"X-User-Role": "admin", "X-Tenant-Id": "1"},
    )
    assert resp.status_code == 200
...
def test_feature_flag_requires_tenant(client_admin, client_no_tenant):
    ...
    resp = client_no_tenant.post(
        "/features/set", json={"name": "abc", "enabled": True}, headers={"X-User-Role": "admin"}
    )
...
def test_unknown_flag_defaults_false(client_admin):
    resp = client_admin.get(
        "/features/check?name=__unknown__", headers={"X-User-Role": "admin", "X-Tenant-Id": "1"}
    )
    assert resp.status_code == 200
    assert resp.json["enabled"] is False
...
def test_commun_builder_projection_shadow_flag_is_registered_and_off(client_admin):
    reg = client_admin.application.feature_registry
    assert reg.has("commun.builder.projection_shadow_v0") is True
    assert reg.enabled("commun.builder.projection_shadow_v0") is False
    reg.set("commun.builder.projection_shadow_v0", True)
    assert reg.enabled("commun.builder.projection_shadow_v0") is True
    reg.set("commun.builder.projection_shadow_v0", False)
    assert reg.enabled("commun.builder.projection_shadow_v0") is False
