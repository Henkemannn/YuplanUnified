...
def test_admin_tenants_list_happy(client_admin):
    resp = client_admin.get(
        "/admin/tenants", headers={"X-User-Role": "superuser", "X-Tenant-Id": "1"}
    )
...
def test_admin_feature_toggle_error_missing_fields(client_admin):
    ...
    resp = client_admin.post(
        "/admin/feature_flags", json={}, headers={"X-User-Role": "superuser", "X-Tenant-Id": "1"}
    )
...
def test_diet_create_type_happy(client_admin):
    resp = client_admin.post(
        "/diet/types",
        json={"name": "Protein Light", "default_select": True},
...
def test_metrics_query_happy(client_admin):
    resp = client_admin.post(
        "/metrics/query", json={"filters": {}}, headers={"X-User-Role": "admin", "X-Tenant-Id": "1"}
    )
    assert resp.status_code == 200
    data = resp.get_json()
...
def test_metrics_summary_missing_from_error(client_admin):
    resp = client_admin.get(
        "/metrics/summary/day", headers={"X-User-Role": "admin", "X-Tenant-Id": "1"}
    )
    assert resp.status_code == 400
    data = resp.get_json()
    assert data["ok"] is False or data.get("error")
...
def test_recommendation_happy(client_admin):
    ...
    resp = client_admin.get(
        "/service/recommendation?category=fish&guest_count=2",
        headers={"X-User-Role": "admin", "X-Tenant-Id": "1"},
    )
...
def test_recommendation_missing_guest_count_error(client_admin):
    resp = client_admin.get(
        "/service/recommendation?category=fish",
        headers={"X-User-Role": "admin", "X-Tenant-Id": "1"},
    )
    assert resp.status_code == 400
    data = resp.get_json()
