...
def test_superuser_can_modify_with_impersonation(client_superuser):
    ...
    if tok:
        headers["X-CSRF-Token"] = tok
    start = client_superuser.post(
        "/superuser/impersonate/start", json={"tenant_id": 1, "reason": "debug"}, headers=headers
    )
    ...
    stop = client_superuser.post(
        "/superuser/impersonate/stop", headers={"X-User-Role": "superuser", "X-Tenant-Id": "1"}
    )
...
def test_support_ui_superuser_only(client_admin, client_superuser):
    ...
    s = client_superuser.get(
        "/admin/support/", headers={"X-User-Role": "superuser", "X-Tenant-Id": "1"}
    )
