...
def test_legacy_cook_create_increments_metric(client_admin):
    ...
    resp = client_admin.post("/tasks/", json={"title": "Cook can create"}, headers={"X-User-Role":"cook","X-Tenant-Id":"1"})
...
def test_viewer_create_does_not_increment(client_admin):
    rec = _Recorder()
    metrics_mod.set_metrics(rec)
    resp = client_admin.post("/tasks/", json={"title": "Viewer blocked"}, headers={"X-User-Role":"viewer","X-Tenant-Id":"1"})
