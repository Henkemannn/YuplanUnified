...
def test_csrf_missing_header_returns_403_problemjson(strict_client):
    r = strict_client.post(
        "/diet/types", json={"name": "A"}, headers={"X-User-Role": "admin", "X-Tenant-Id": "1"}
    )
...
def test_csrf_invalid_token_returns_403_problemjson(strict_client):
    ...
    r = strict_client.post(
        "/diet/types",
        json={"name": "B"},
        headers={"X-User-Role": "admin", "X-Tenant-Id": "1", "X-CSRF-Token": "bad"},
    )
    assert r.status_code == 403
    j = r.get_json()
...
def test_csrf_valid_token_allows_post(strict_client):
    ...
    r = strict_client.post(
        "/diet/types",
        json={"name": "C"},
        headers={"X-User-Role": "admin", "X-Tenant-Id": "1", "X-CSRF-Token": tok},
    )
...
def test_csrf_feature_flag_off_no_enforcement(client_admin):
    ...
    r = client_admin.post(
        "/diet/types", json={"name": "D"}, headers={"X-User-Role": "admin", "X-Tenant-Id": "1"}
    )
