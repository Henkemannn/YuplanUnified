...
def test_cook_warn_once_per_tenant_day(monkeypatch, client_admin, caplog):
    ...
    r1 = client_admin.post(
        "/tasks/", json={"title": "A"}, headers={"X-User-Role": "cook", "X-Tenant-Id": "1"}
    )
    assert r1.status_code in (200, 201)
    # second create same day
    r2 = client_admin.post(
        "/tasks/", json={"title": "B"}, headers={"X-User-Role": "cook", "X-Tenant-Id": "1"}
    )
...
def test_viewer_no_warn(monkeypatch, client_admin, caplog):
    ...
    caplog.set_level("WARNING")
    r = client_admin.post(
        "/tasks/", json={"title": "V"}, headers={"X-User-Role": "viewer", "X-Tenant-Id": "1"}
    )
