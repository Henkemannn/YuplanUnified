...
def test_upsert_emits_metric(monkeypatch):
    ...
    c = app.test_client()
    c.post("/admin/limits", headers={"X-User-Role":"admin"}, json={"tenant_id":1,"name":"exp","quota":5,"per_seconds":60})
...
def test_delete_emits_metric(monkeypatch):
    ...
    c.delete("/admin/limits", headers={"X-User-Role":"admin"}, json={"tenant_id":2,"name":"exp"})
