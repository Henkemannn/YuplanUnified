...
def test_notes_update_requires_elevation_for_non_owner_marker(client_admin):
    ...
    create = client_admin.post("/notes/", json={"content": "abc"}, headers={"X-User-Role": "admin", "X-Tenant-Id": "1"})
    assert create.status_code == 200
    note_id = create.get_json()["note"]["id"]
    r = client_admin.put(f"/notes/{note_id}", json={"content": "zzz"}, headers={"X-User-Role": "admin", "X-Tenant-Id": "1"})
    assert r.status_code == 200
