...
import sqlite3
import os
import datetime as dt_module  # keep module alias if needed elsewhere
import json
...
def inject_global_vars():
    return {
        # använder modulalias för konsekvens
...
# Central helper för aktuell ISO-vecka (använd alltid denna istället för direkt datetime-anrop i vyer)
def get_current_week():
    try:
        return dt_module.date.today().isocalendar()[1]
    except Exception:
        # Fallback: om något märkligt händer, använd vecka 1
        return 1
...
def menyval():
    ...
    # Använd central helper istället för direkt datetime-anrop (fixar AttributeError pga namnkonflikt)
    vecka = request.args.get('vecka', default=get_current_week(), type=int)
...
def get_default_week():
    ...
    try:
        ...
        # Använder modulalias dt_module för att undvika namnkonflikter
        return dt_module.datetime.now().isocalendar().week
    except Exception:
        return 1
